...
public class ProductRepository {
    ...
    public void getAllProductsCategory(String lang, String token, int idTitle, int idCategory, ResponseServer<LiveData<ProductsResponse>> responseServer) {
        MutableLiveData<ProductsResponse> products = new MutableLiveData<>();
        ApiService apiService = ApiClient.getClient().create(ApiService.class);
        apiService.getAllProductsCategory(lang, token, idTitle, idCategory).enqueue(new Callback<ProductsResponse>() {
            @Override
    ...
    public void getAllProductsCompany(String lang, String token, int idTitle, int idCompany, ResponseServer<LiveData<ProductsResponse>> responseServer) {
        MutableLiveData<ProductsResponse> products = new MutableLiveData<>();
        ApiService apiService = ApiClient.getClient().create(ApiService.class);
        apiService.getAllProductsCompany(lang, token, idTitle, idCompany).enqueue(new Callback<ProductsResponse>() {
            @Override
            public void onResponse(@NonNull Call<ProductsResponse> call, @NonNull Response<ProductsResponse> response) {
                products.setValue(response.body());
                responseServer.onSuccess(response.isSuccessful(), response.code(), products);
    ...
    public void addOrRemoveProductFavorite(String lang, String token, int idProduct, ResponseServer<LiveData<MessageResponse>> responseServer) {
        MutableLiveData<MessageResponse> message = new MutableLiveData<>();
        ApiService apiService = ApiClient.getClient().create(ApiService.class);
        apiService.addOrRemoveProductFavorite(lang, token, idProduct).enqueue(new Callback<MessageResponse>() {
            @Override
